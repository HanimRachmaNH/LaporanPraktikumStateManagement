...
class University {
  ...
  University({
    required this.name, // Constructor untuk inisialisasi properti nama universitas
    required this.website, // Constructor untuk inisialisasi properti alamat website universitas
  });
...
class UniversityEvent extends Equatable {
  ...
  @override
  List<Object?> get props => [country]; // Mendefinisikan properti yang akan digunakan untuk perbandingan objek
}
...
class UniversityState extends Equatable {
  ...
  @override
  List<Object> get props => [universities]; // Mendefinisikan properti yang akan digunakan untuk perbandingan objek
}
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          yield UniversityState(
              universities: universities); // Mengirimkan state baru dengan daftar universitas yang diperbarui
        } else {
          throw Exception('Failed to load university data');
        }
      } catch (e) {
        yield UniversityState(universities: []); // Mengirimkan state baru dengan daftar universitas kosong jika terjadi kesalahan
      }
    }
  }
}
        ...
        builder: (context, state) {
          if (state is UniversityState) {
            if (state.universities.isEmpty) {
              return Center(
                  child: CircularProgressIndicator()); // Menampilkan indikator progres jika daftar universitas kosong
            } else {
